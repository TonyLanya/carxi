<?php
 ...
 //                    $row_result_trips[0]['fDiscount']=round($row_result_trips[0]['fDiscount'] * $row_result_trips[0]['fRatioDriver'],1);
	...
	if($type=="driversignup"){
		...
		$vPassword         = isset($_REQUEST["password"]) ? $_REQUEST["password"] : '';
		$iGcmRegId        = isset($_REQUEST["device_token"]) ? $_REQUEST["device_token"] : '';	
		$deviceType = isset($_REQUEST["device_type"]) ? $_REQUEST["device_type"] : 'Android';
		...
		$check_driver    = $obj->MySQLSelect($sql);

		$Password_driver = $generalobj->encrypt($vPassword);
		...
		if(count($check_driver)>0){
			...
			$Data_passenger['vFirstName']=$vFirstName;
			$Data_passenger['vLastName']=$vLastName;
			$Data_passenger['vEmail']=$vEmail;
			$Data_passenger['vCity']=$vCity;
			$Data_passenger['vPhone']=$vPhone;
			$Data_passenger['vPassword']=$Password_driver;
			$Data_passenger['iGcmRegId']=$iGcmRegId;
			$Data_passenger['eDeviceType']=$deviceType;
			$Data_passenger['vCountry']=$vCountry;
			$Data_passenger['eRefType']=$user_type;
			$Data_passenger['vCompany']=$vCompany;
			$Data_passenger['vCaddress']=$vCaddress;
			$Data_passenger['vCadress2']=$vCadress2;
			$Data_passenger['vZip']=$vZip;
			$Data_passenger['vCurrencyDriver']=$vCurrencyDriver;
			$Data_passenger['dBirthDate']=$dBirthDate;
			$Data_passenger['vLatitude']=$vLatitude;
			$Data_passenger['vLongitude']=$vLongitude;
			$Data_passenger['vLongitude']=$vLongitude;
			$Data_passenger['vLang']= get_value('language_master', 'vCode', 'eDefault','Yes','','true');
           ...
           $Data_passenger['eDeviceType']=$deviceType;
			...
			$Data_passenger['dRefDate'] =  Date('Y-m-d H:i:s');
			$id = $obj->MySQLQueryPerform("register_driver",$Data_passenger,'insert');
			...
			if ($id > 0) {
				...
				/*new added*/
				$returnArr['Action'] = "1";
				$returnArr['driver_id'] = $id;
				$returnArr['message'] = "driver register successfully";
	...
	if($type=="documentUpload"){
		...
		$vName          = isset($_REQUEST["name"]) ? $_REQUEST["name"] : '';
		$vEmail        = isset($_REQUEST["email"]) ? $_REQUEST["email"] : '';
		$vCity         = isset($_REQUEST["city"]) ? $_REQUEST["city"] : '';
		$vPhone     = isset($_REQUEST["phone_no"]) ? $_REQUEST["phone_no"] : '';
		$vPassword         = isset($_REQUEST["password"]) ? $_REQUEST["password"] : '';
		$iGcmRegId        = isset($_REQUEST["device_token"]) ? $_REQUEST["device_token"] : '';
	...
	if ($type == "signIn") {
		...
		if($UserType == "Passenger"){
			$sql = "SELECT iUserId,eStatus,vLang,vTripStatus FROM `register_user` WHERE vEmail='$Emid'  && vPassword='$Password_user'";
			$Data = $obj->MySQLSelect($sql);
			...
			//$sql = "SELECT rd.iDriverId,rd.eStatus,rd.vLang,cmp.eStatus as cmpEStatus FROM `register_driver` as rd,`company` as cmp WHERE rd.vEmail='$Emid'  AND rd.vPassword='$Password_user' AND cmp.iCompanyId=rd.iCompanyId";
			 $sql = "SELECT * FROM `register_driver` WHERE vEmail='$Emid'  AND vPassword='$Password_user'";
			$Data = $obj->MySQLSelect($sql);
	...
	if($type=='estimateFare'){
		...
		/* custom calculation of fare on basis of distance(new code) */
		// $d1 = $distance/2;
		// $d2 = $distance/4;
		// $distance_in_miles = round($d1)+round($d2); //distance in miles
		...
		{
			$newDist = $distance-2.5;
			$myDistPrice = $newDist*2;
		}
		else
		{
			$myDistPrice = 7;
		...
		$Fare_data[0]['total_fare'] = $myDistPrice+7; //number_format(round($Fare_data[0]['total_fare'] * $priceRatio,1),2);
		...
		$Fare_data[0]['fPricePerKM'] = $myDistPrice; //number_format(round($myDistPrice * $priceRatio,1),2);
  ...
  if ($type == "getRideHistory") {
	...
	if($type == 'testnoti')
	{
		...
		$PickUpLatitude    = '30.7333148';
		$PickUpLongitude    ='76.7794179';

		$DestLatitude    = '30.7046486';
		$DestLongitude    = '76.71787259999996';
		$DestAddress    = 'Mohali, Punjab, India';
		$promoCode    =isset($_REQUEST["PromoCode"]) ? $_REQUEST["PromoCode"] : '';
		$eType    = 'Ride';
		$iPackageTypeId    = isset($_REQUEST["iPackageTypeId"]) ? $_REQUEST["iPackageTypeId"] : '';
		$vReceiverName    = 'Beco';
		$vReceiverMobile    = '9501318140';
		...
		$vDeviceToken    = 'cCSj8csrd9I:APA91bFpMZNI4fxFPlUO9BstDlyh8Lef9HLtycz7AGltucaWZ51L2uDlVV5ytbMyuaOgVk-fiCURFSFH1v_M6g9AEL1NvC8Pe7H5CbXFGbwMf5gPQRqCB2ZvvVK3lmi5zUuHHj_mbYRo';
...
#        }
    ...
    $result_fare['FormattedTripDate']=date('dS M \a\t h:i a',strtotime($result_fare[0]['tStartDate']));
	...
	if($type == "DriverArrived"){
		...
		if($iDriverId != '') {
			...
			if($id>0){
				...
				$sql = "SELECT tr.tEndLat,tr.tEndLong,tr.tDaddress,tr.iUserId FROM trips as tr,register_driver as rd WHERE tr.iTripId=rd.iTripId AND rd.iDriverId = '".$iDriverId."'";
				$result = $obj->MySQLSelect($sql);
	...
	if($type == "ProcessEndTrip"){
        ...
        $tripStopsTime   = isset($_REQUEST["tripStopsTime"]) ? $_REQUEST["tripStopsTime"] : 0;
        
        $ctrStopInterval = 0;
        //$ctrStopInterval = count($tripStopsTime);
        $ctrStopInterval = $tripStopsTime;
		...
		$Fare_data=calculateFareWithStopTime($totalTimeInMinutes_trip,$tripDistance,$vehicleTypeID,$userId,1,$startDate,$endDateOfTrip,$couponCode,$tripId, $ctrStopInterval);
        
    $where = " iTripId = '" . $tripId . "'";
        
        $stopFare = $tripStopsTime * 3.0;
        $iFare = $Fare_data['total_fare'];// + $stopFare;
    ...
    $Data_update_trips['tStopTime']=$tripStopsTime;
        
        $Data_update_trips['fStopFare']=$stopFare;
		if($isTripCanceled == "true"){
			$Data_update_trips['vCancelReason'] = $driverReason;
			$Data_update_trips['vCancelComment'] = $driverComment;
			$Data_update_trips['eCancelled'] = "Yes";
...
$sql = "SELECT tr.*, rate.vRating1, rate.vMessage,ru.vName,ru.vLastName,ru.vImgName as vImage  FROM trips tr LEFT JOIN ratings_user_driver rate ON (rate.iTripId = tr.iTripId AND rate.eUserType='Passenger') LEFT JOIN register_user ru ON ru.iUserId=tr.iUserId WHERE tr.iActive = 'Finished' AND tr.iDriverId = '$iDriverId' AND tr.tTripRequestDate LIKE '$date%'";
		$tripData = $obj->MySQLSelect($sql);
		...
		if(count($tripData) > 0){
			...
			for($i=0;$i<count($tripData);$i++){
				...
				$totalEarnings = $totalEarnings + $tripData[$i]['iFare'];		//Pankaj Code
				$avgRating = $avgRating + $vRating1;

        $returnArr = getTripPriceDetails($tripData[$i]['iTripId'],$iDriverId,"Driver");
        $returnArr['iFare'] = $tripData[$i]['iFare'];							//Pankaj Code
	...
	if ($type == "checkBookings") {
		...
		if ( count($Data) > 0 ) {
			...
			for($i=0;$i<count($Data);$i++){
				$Data[$i]['dBooking_date']=date('dS M \a\t h:i a',strtotime($Data[$i]['dBooking_date']));
			}
			$returnArr['Action'] ="1";
			$returnArr['message'] =$Data;
