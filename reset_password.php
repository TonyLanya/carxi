<?php
	...
	if($_POST['submit']){	
		...
		$_POST['time']=(base64_decode(base64_decode(trim($_POST['time']))));
		$success=(isset($_REQUEST['success'])?$_REQUEST['success']:'');
		...
		if($type == 'rider'){		
			...
			header("Location:reset_password.php?type=".$type."&id=".$id."&time=".$time."&&success=1&var_msg=".$var_msg); exit;
			}
		...
		if($tablename !='' && $type_action!='' && $id !=''){			
			if($newpassword == $vPassword){				
				 $sql = "UPDATE ".$tablename." set vPassword='".$generalobj->encrypt($vPassword)."' WHERE ".$filed_Id."='".$id."'";  
				$obj->sql_query($sql);				
							...
							if($total_hours <=24){									
							 ...
							 <? } ?>
								...
								<div class="login-form-left reset-password-page">
									...
									<input type="hidden" name="time" value="<? echo base64_encode(base64_encode($time));?>"/>   	
										...
										<input name="newpassword" type="text"  id="newpassword" placeholder="<?=$langage_lbl['LBL_NEW_PASSWORD_TXT']; ?>" class="login-input" value="" required /></b>
										</span>
										<span class="newrow">
										<b>
