<?php
    ...
    if($type=="signIn_User"){
		...
		$email_field = " vEmail = '".$email."'";
		$password_field = " AND `vPassword` = '".$password."'";
            $sql = "SELECT iDriverId, vLang, eStatus FROM `register_driver` WHERE vEmail = '".$email."' AND `vPassword` = '".$password."' ";//. $email_field . $password_field;
            $Data = $obj->MySQLSelect($sql);
		}
  ...
  if ($type == "getRideHistory") {
    ...
    $userwaitinglabel = $languageLabelsArr['LBL_TRIP_USER_WAITING'];
        ...
        $sql = "SELECT iGcmRegId,eDeviceType FROM register_driver WHERE iDriverId IN (".$driver_id_auto.") AND tLastOnline > '$str_date' AND vAvailability='Available'";
...
//        $message      = "TripEnd";
        ...
        $Data_update_trips['fStopFare']=$Fare_data['fStopFare'];
   	$Data_update_trips['vDiscount'] =$Fare_data['vDiscount'] ;
        ...
        if($id>0){
            ...
            $returnArr['StopTimeFare']=strval(round($Fare_data['fStopFare'] * $trip_start_data_arr[0]['fStopFare'],1));
  ...
  /*  if($type=='displayFareToPassenger'){
    ...
    if($type=='estimateFare'){
        ...
        // echo round($Fare_data[0]['total_fare'] * $priceRatio);
		//        echo $Fare_data[0]['total_fare'];
        $Fare_data[0]['Action'] = "1";
        $Fare_data[0]['Distance']= $resultArr['Distance'] ==NULL ? "0" : strval(round($resultArr['Distance'],2));
        $Fare_data[0]['Time']= $resultArr['Time'] == NULL ? "0" : strval(round($resultArr['Time'],2));
        ...
        $Fare_data[0]['total_fare'] = strval(round($Fare_data[0]['total_fare'] * $priceRatio,1));
	...
	if($type == "getDriverRideHistory"){
		...
		if(count($tripData) > 0){
			...
			for($i=0;$i<count($tripData);$i++){
				...
				$fStopFare = $tripData[$i]['fStopFare'];
  ...
  if($type=="createTokboxsession"){

    $link = mysqli_connect("localhost", "carxi_carxi", "=C,T*5G_(o_z", "carxi_carxi");
...
exit;
    ...
    if($type == "pushNotification"){
        ...
        //$passphrase = '123456';		//Comment by Pankaj
        $passphrase = '1234';			//Pankaj code
...
//        stream_context_set_option($ctx, 'ssl', 'local_cert', 'apn-dev-uberapp.pem');
        //stream_context_set_option($ctx, 'ssl', 'local_cert', 'driver_apns_dev.pem');	//Comment by  Pankaj
        stream_context_set_option($ctx, 'ssl', 'local_cert', 'carxiDriverDevMac.pem');	//Pankaj .pem file
        stream_context_set_option($ctx, 'ssl', 'passphrase', $passphrase);

        // Open a connection to the APNS server
        $fp = stream_socket_client(
		//'ssl://gateway.sandbox.push.apple.com:2195', $err,		//Comment by Pankaj
