		...
		<div id="wrap">
			...
			<div id="content">
				<div class="inner">
					<div class="row">
						<div class="col-lg-12">
							<h2><?= ucfirst($action); ?> Document of <?= $_REQUEST['vehicle'];?> </h2>
							<a href="vehicles.php">
									...
									<div class="panel panel-default upload-clicking">
										...
										<div class="panel-body">
											...
											<?php $file_ext = $generalobj->file_ext($db_user[0]['vInsurance']);
												...
												<img src = "<?= $tconfig["tsite_upload_vehicle_doc_panel"] . '/' .$_REQUEST['id'].'/'. $db_user[0]['vInsurance'] ?>" style="width:200px;" alt ="Insurance not found"/>
												<?php }else{ ?>
												<a href="<?= $img_path  . '/' .$_REQUEST['id'].'/'. $db_user[0]['vInsurance']  ?>" target="_blank">Insurance File</a>
											<?php } ?>
											<?php } else { ?>
											Need to upload
											<?php } ?>
											...
											<?php if ($db_user[0]['vInsurance'] != '') { 
												echo $langage_lbl_admin['LBL_DOC_UPDATE_INS'];
												}else{
													echo $langage_lbl_admin['LBL_DOC_ADD_INS'];
												}?>
											</button>
                                            </b>
										</div>
									...
									<div class="panel panel-default upload-clicking">
										...
										<div class="panel-body">
											...
											<?php $file_ext = $generalobj->file_ext($db_user[0]['vPermit']);
												if($file_ext == 'is_image'){ ?>
												<img src = "<?= $tconfig["tsite_upload_vehicle_doc_panel"] . '/' . $_REQUEST['id'].'/'.$db_user[0]['vPermit'] ?>" style="width:200px;" alt ="Permit not found"/>
												<?php }else{ ?>
												<a href="<?= $img_path  . '/'. $_REQUEST['id'].'/'. $db_user[0]['vPermit']  ?>" target="_blank">PERMIT File</a>
											<?php } ?>
											<?php } else { ?>
											Need to upload
											<?php } ?>
											...
											<button class="btn btn-info" data-toggle="modal" data-target="#uiModal_3">
												...
												<?php if ($db_user[0]['vPermit'] != '') { 
												echo $langage_lbl_admin['LBL_DOC_UPDATE_PER'];
												}else{
													echo $langage_lbl_admin['LBL_DOC_ADD_PER'];
												}?>
											</button>
                                            </b>
										</div>
										...
										<div class="panel-body">
											<?php
												...
												<?php $file_ext = $generalobj->file_ext($db_user[0]['vRegisteration']);
													if($file_ext == 'is_image'){ ?>
													<img src = "<?= $tconfig["tsite_upload_vehicle_doc_panel"] . $_REQUEST['id'].'/'.
													 $db_user[0]['vRegisteration'] ?>" style="width:200px;" alt ="REGISTRATION not found"/>
													<?php }else{ ?>
													<a href="<?= $tconfig["tsite_upload_vehicle_doc_panel"]. '/' .$_REQUEST['id'].'/'.$db_user[0]['vRegisteration']  ?>" target="_blank"> File</a>
												<?php } ?>
												<?php } else { ?>
												Need to upload
											<? } ?>
											...
											<button class="btn btn-info" data-toggle="modal" data-target="#uiModal_4" >
												...
												echo $langage_lbl_admin['LBL_DOC_UPDATE_REG'];
												}else{
													echo $langage_lbl_admin['LBL_DOC_ADD_REG'];
												}?>
											</button>
                                            </b>
										</div>
																...
																<div class="fileupload-preview thumbnail" style="width: 200px; height: 150px; ">
																	<?php if ($db_user[0]['vInsurance'] == '') { ?>
																		...
																		<?php $file_ext = $generalobj->file_ext($db_user[0]['vInsurance']);
																			...
																			<img src = "<?= $tconfig["tsite_upload_vehicle_doc_panel"] . $_REQUEST['id'].'/' . $db_user[0]['vInsurance'] ?>" style="width:200px;" alt ="Insurance not found"/>
																			<?php }else{ ?>
																			<a href="<?= $tconfig["tsite_upload_vehicle_doc_panel"] .$_REQUEST['id'].  '/' . $db_user[0]['vInsurance'] ?>" target="_blank">Insurance File</a>
																		<?php } ?>
																	<?php } ?>
																</div>
																<div>
																	<span class="btn btn-file btn-success"><span class="fileupload-new">Upload Insurance</span><span class="fileupload-exists">Change</span><input type="file" name="noc"/></span>
																...
																<div class="fileupload fileupload-new" data-provides="fileupload">
																	<div class="fileupload-preview thumbnail" style="width: 200px; height: 150px; ">
																		<?php if ($db_user[0]['vRegisteration'] == '') { ?>
																			...
																			<?php $file_ext = $generalobj->file_ext($db_user[0]['vRegisteration']);
																				if($file_ext == 'is_image'){ ?>
																				<img src = "<?= $tconfig["tsite_upload_vehicle_doc_panel"] .$_REQUEST['id']. '/' . $db_user[0]['vRegisteration'] ?>" style="width:200px;" alt ="Insurance not found"/>
																				<?php }else{ ?>
																				<a href="<?= $tconfig["tsite_upload_vehicle_doc_panel"].$_REQUEST['id'] .  '/' . $db_user[0]['vRegisteration'] ?>" target="_blank">Registration File</a>
																			<?php } ?>
																		<?php } ?>
																	</div>
																	<div>
																		<span class="btn btn-file btn-success"><span class="fileupload-new">Upload Registration</span><span class="fileupload-exists">Change</span><input type="file" name="reg"/></span>
																		<a href="#" class="btn btn-danger fileupload-exists" data-dismiss="fileupload">Remove</a>
																	</div>
																</div>
															</div>
																	...
																	<div class="fileupload fileupload-new" data-provides="fileupload">
																		<div class="fileupload-preview thumbnail" style="width: 200px; height: 150px; ">
																			<?php if ($db_user[0]['vPermit'] =='') { ?>
																				...
																				<?php $file_ext = $generalobj->file_ext($db_user[0]['vPermit']);
																					if($file_ext == 'is_image'){ ?>
																					<img src = "<?= $tconfig["tsite_upload_vehicle_doc_panel"].$_REQUEST['id']. '/'. $db_user[0]['vPermit'] ?>" style="width:200px;" alt ="NOC not found"/>
																					<?php }else{ ?>
																					<a href="<?=  $path .$_REQUEST['id'].'/' . $db_user[0]['vPermit']  ?>" target="_blank">NOC File</a>
																				<?php } ?>
                                        <?php } ?>
																		</div>
																		<div>
