<?php
	...
	$ToDate = date('Y-m-d',strtotime($CRON_TIME));
	$sql1 = "SELECT iCabBookingId,iCronStage,eAssigned FROM cab_booking WHERE eStatus='Assign' AND dBooking_date LIKE '%$ToDate%' AND eAutoAssign = 'Yes' AND iCronStage != '3' AND eAssigned='No'";
	...
	for($i=0;$i<count($data_bks);$i++){
		...
		if($data_bks[$i]['iCronStage'] == 1) {
			$ToDate = @date('Y-m-d H:i:s', strtotime('-6 minutes'));
		}
		...
		WHERE cb.eStatus='Assign' AND cb.dBooking_date <= '$ToDate' AND cb.dBooking_date >= '$FromDate' AND cb.iCabBookingId='".$data_bks[$i]['iCabBookingId']."'";
		...
		if(count($data_booking)) {
			...
			$Data2 = array();
			$Data2 = getOnlineDriverArr($vSourceLatitude,$vSourceLongitude);
