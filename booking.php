		...
		<div id="main-uber-page">
			...
			<div class="page-contant">
				<div class="page-contant-inner">
					<h2 class="header-page trip-detail driver-detail1">My Bookings<a href="javascript:void(0);" onClick="manual_dispatch_form();">Add Booking</a></h2>
					<!-- trips page -->
									...
									<table width="100%" border="0" cellpadding="0" cellspacing="0" id="dataTables-example">
										<thead>
											<tr>
												...
												<th width="20%">Date</th>
												<th>Pickup</th>
												<th width="10%">Destintion</th>
												<th width="15%" style="width: 67px;"><?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN'];?></th>
												<th width="14%"><?php echo $langage_lbl_admin['LBL_TRIP_TXT_ADMIN'];?>  Details</th>
										...
										<tbody>
											<? for ($i = 0; $i < count($data_drv); $i++) { ?>
												<tr class="gradeA">
													...
													<td><a class="btn btn-info" href="manual_dispatch.php?booking_id=<?= $data_drv[$i]['iCabBookingId']; ?>"><i class="icon-shield icon-flip-horizontal icon-white"></i> Assign Driver</a><br>( Car Type : <?= $data_drv[$i]['vVehicleType']; ?>)</td>
													<?php } else if($data_drv[$i]['eCancelBy'] == "Driver" && $data_drv[$i]['eStatus'] == "Cancel") { ?>
													<td><a class="btn btn-info" href="manual_dispatch.php?booking_id=<?= $data_drv[$i]['iCabBookingId']; ?>"><i class="icon-shield icon-flip-horizontal icon-white"></i> Assign Driver</a><br>( Car Type : <?= $data_drv[$i]['vVehicleType']; ?>)</td>
													<?php } else if ($data_drv[$i]['driver'] != "" && $data_drv[$i]['driver'] != "0") { ?>
													<td><b><?= $data_drv[$i]['driver']; ?></b><br>( Car Type : <?= $data_drv[$i]['vVehicleType']; ?>) </td>
													<?php } else  { ?>
													<td>---<br>( Car Type : <?= $data_drv[$i]['vVehicleType']; ?>)</td>
													<?php } ?>
													<td><?php if($data_drv[$i]['iTripId'] != "" && $data_drv[$i]['eStatus'] == "Completed") { ?><a class="btn btn-primary" href="invoice.php?iTripId=<?=$data_drv[$i]['iTripId'];?>">View</a><?php }else {echo "---"; } ?></td>
