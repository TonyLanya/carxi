...
$curdate=date("Y-m-d");
$input_date=$db_user[0]['dLicenceExp'];
...
//}
if($image_name=="" && $db_user[0]['dLicenceExp']!=$curdate)
{
   $tbl = 'register_driver';
   $q = "UPDATE ";
    $where = " WHERE `iDriverId` = '" . $_SESSION['sess_iUserId'] . "'";
  ...
  exit;
     ...
     if ($image_name != "") { 
         // $check_file_query = "select iCompanyId,vLicence from company where iCompanyId=" . $iMemberId;
          $check_file_query = "select iDriverId,vLicence from register_driver where iDriverId=" . $_SESSION['sess_iUserId'];
          $check_file = $obj->sql_query($check_file_query);
          $check_file['vLicence'] = $doc_path . '/' . $_SESSION['sess_iUserId'] . '/' . $check_file[0]['vLicence'];
	...
	`iDriverId` = '" . $_SESSION['sess_iUserId'] . "'" . $where;
			...
			//////////////////////////
              //Start :: Log Data Save
                         if(empty($check_file[0]['vLicence'])){ $vLicencePath = $vImage ; }else{ $vCertiPath = $check_file[0]['vLicence']; }
                         $generalobj->save_log_data ('0',$_SESSION['sess_iUserId'],'driver','licence',$vLicencePath);
              //End :: Log Data Save
